...
'''
    ...
    try:
        ...
        print(f"OpenRouter API response: {response}") # Log the full response
        return jsonify({"correct": False, "explanation": f"An error occurred during evaluation: {str(e)}. Check server logs for details."}), 500
...
def get_solution():
    ...
    try:
        response = requests.post(
            ...
            data=json.dumps({
                "model": "deepseek/deepseek-r1-0528-qwen3-8b:free",
                "messages": [{"role": "user", "content": f"Provide a Python solution for the following problem:\n{question}"}]
            })
        ).json()
        ...
        print(f"OpenRouter API response: {response}") # Log the full response
        return jsonify({"error": f"An error occurred during solution retrieval: {str(e)}. Check server logs for details."}), 500
